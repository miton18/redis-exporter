...
import (
	...
	core "github.com/miton18/redis-exporter/core"
)
...
func initConfig() {
	...
	viper.SetEnvPrefix("redisexport")
	viper.AutomaticEnv()

	// Set config search path
	viper.AddConfigPath("/etc/redis-exporter/")
	viper.AddConfigPath("$HOME/.redis-exporter")
	viper.AddConfigPath(".")
...
// Source defined a REDIS stats source
type Source struct {
	Address string
...
var RootCmd = &cobra.Command{
	Use:   "redis-exporter",
	Short: "Redis exporter expose Redis stats",
	Run: func(cmd *cobra.Command, args []string) {
		log.Info("Redis exporter starting")
		...
		for i := range exporters {
			...
			exporter, err := core.NewExporter(s.Address,
				labels,
